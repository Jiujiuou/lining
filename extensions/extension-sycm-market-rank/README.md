...
2. **content.js** 解析结构同 `response.json`：`data.data.data.data[]`，取 **`shop.title`（店铺名）**、**`item.title`（商品名）**、`item.itemId`（勾选键）、`cateRankId.value`，写入扩展日志并 `chrome.runtime.sendMessage` 交给 background。
3. **background.js** 保存列表快照后，按 **popup 中已保存的勾选**（`itemId` 或 `idx-行号`）过滤行，将 **`shop_title`、`rank`、`item_title`（商品名，可空）** 批量 **POST** 到 Supabase 表 **`sycm_market_rank_log`**（建表见本目录 **`supabase_sycm_market_rank_log.sql`**；已有表需执行 **`sql/migrate_sycm_market_rank_log_item_title.sql`** 增加 `item_title` 列）。
