(function () {
    ...
    try {
      document.dispatchEvent(
        new CustomEvent('sycm-flow-source', {
          detail: { payload: payload, recordedAt: getEast8TimeStr(), itemId: String(itemId) },
        })
      );
    } catch (e) {}
  }
    ...
    return fetch(url, { method: 'GET', credentials: 'include' })
      ...
      .then(function (r) {
        if (!r.ok || !r.json || r.json.code !== 0) {
          ...
          var message =
            (r.json && (r.json.message || r.json.msg || r.json.subMsg)) ? String(r.json.message || r.json.msg || r.json.subMsg) : '';
          var msg =
            'item ' +
            String(itemId) +
            ' 请求失败：HTTP ' +
    ...
    while (inFlight < maxConcurrency && queue.length > 0) {
      ...
      fetchOne(itemId)
        ...
        .catch(function (err) {
          ...
          stopInternal('请求失败，请打开任意商品详情页刷新模板后重试。错误：' + String(err && err.message || err));
        })
        .finally(function () {
          inFlight--;
  ...
  function start(opts) {
    ...
    intervalMs = typeof opts.intervalMs === 'number' && opts.intervalMs >= 5000 ? opts.intervalMs : intervalMs;
    ...
    emitLog('log', '[Sycm][轮询] 已开始：商品 ' + ids.length + ' 个，间隔 ' + Math.round(intervalMs / 1000) + ' 秒，并发 ' + maxConcurrency);
    tick();
  }
