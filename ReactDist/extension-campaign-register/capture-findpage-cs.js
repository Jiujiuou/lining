(function() {
  ...
  function initFindPageCaptureContent() {
    if (globalThis.__LINING_AMCR_CS__) {
      return;
    }
    globalThis.__LINING_AMCR_CS__ = true;
    var STATE_BY_TAB = "amcr_findPageStateByTab";
    var MAX_FINDPAGE_TABS = typeof __AMCR_DEFAULTS__ !== "undefined" && __AMCR_DEFAULTS__.FIND_PAGE_MAX_TABS ? __AMCR_DEFAULTS__.FIND_PAGE_MAX_TABS : 6;
    ...
    function sendCaptureLog(tabId, msg) {
      try {
        chrome.runtime.sendMessage({ type: "AMCR_CAPTURE_LOG", tabId: tabId != null ? tabId : null, msg }, function() {
        });
      } catch (e) {
      }
    }
    ...
    function slimFindPagePayload(payload) {
      ...
      try {
        ...
        return {
          data: {
            count: payload.data && payload.data.count != null ? payload.data.count : slimList.length,
            list: slimList
          }
        };
      } catch (e) {
    ...
    function extractCampaignList(payload) {
      ...
      if (payload.data && payload.data.data && Array.isArray(payload.data.data.list)) {
    ...
    function onMessage(event) {
      ...
      try {
        ...
        resolveTabId(function(tabId) {
          sendCaptureLog(
            tabId,
            "已接收捕获消息：" + list.length + " 条，biz=" + (biz || "unknown") + "，url=" + requestUrl
          );
...
//# sourceMappingURL=capture-findpage-cs.js.map
