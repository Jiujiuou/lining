(function() {
  ...
  function getSlotKey(recordedAtStr, throttleMinutes = DEFAULT_THROTTLE_MINUTES) {
  ...
  const SYCM_RANK_SUPABASE = {
    url: "https://ijfzeummbriivdmnhpsi.supabase.co",
    anonKey: "sb_publishable_JLdtAikiQsgtXeKPJ_Gfrg_bkeBNHh_"
  };
  ...
  function slimRankPayload(rawPayload) {
    ...
    const nextItems = [];
    for (let i = 0; i < sourceItems.length && nextItems.length < SYCM_RANK_LIMITS.RANK_MAX_ITEMS; i += 1) {
    ...
    return {
      ...
      recordedAtEast8: rawPayload.recordedAtEast8 != null ? String(rawPayload.recordedAtEast8) : "",
      items: nextItems,
  ...
  function appendLogByTab(tabId, level, message) {
    ...
    if (tabId == null) {
      getLocalAsync([SYCM_RANK_STORAGE_KEYS.logs]).then((result) => {
        const data = result[SYCM_RANK_STORAGE_KEYS.logs] && typeof result[SYCM_RANK_STORAGE_KEYS.logs] === "object" ? { ...result[SYCM_RANK_STORAGE_KEYS.logs] } : { entries: [] };
    ...
    getLocalAsync([SYCM_RANK_STORAGE_KEYS.logsByTab]).then((result) => {
      const byTab = result[SYCM_RANK_STORAGE_KEYS.logsByTab] && typeof result[SYCM_RANK_STORAGE_KEYS.logsByTab] === "object" ? { ...result[SYCM_RANK_STORAGE_KEYS.logsByTab] } : {};
  ...
  function buildSupabaseRows(payload, selectedIds) {
    ...
    for (let i = 0; i < source.length; i += 1) {
      ...
      rows.push({
        shop_title: row.shopTitle != null && String(row.shopTitle).trim() !== "" ? String(row.shopTitle).trim() : "（无店名）",
        rank: row.rank != null ? Number(row.rank) : 0,
        item_title: row.itemTitle != null && String(row.itemTitle).trim() !== "" ? String(row.itemTitle).trim() : null
      });
    }
    return rows;
  }
  async function uploadRankToSupabase(tabId, payload, done) {
    ...
    const minutes = store[SYCM_RANK_STORAGE_KEYS.throttleMinutes] != null && Number(store[SYCM_RANK_STORAGE_KEYS.throttleMinutes]) > 0 ? Number(store[SYCM_RANK_STORAGE_KEYS.throttleMinutes]) : SYCM_RANK_LIMITS.DEFAULT_THROTTLE_MINUTES;
    const selectedFilter = tabId != null && store[SYCM_RANK_STORAGE_KEYS.rankSelectionByTab] && store[SYCM_RANK_STORAGE_KEYS.rankSelectionByTab][String(tabId)] ? store[SYCM_RANK_STORAGE_KEYS.rankSelectionByTab][String(tabId)] : store[SYCM_RANK_STORAGE_KEYS.rankSelection];
    const selectedIds = selectedFilter && Array.isArray(selectedFilter.itemIds) ? selectedFilter.itemIds.map((id) => String(id)) : [];
    ...
    const slotStorageKey = SYCM_RANK_STORAGE_KEYS.lastSlotPrefix + `sycm-market-rank_${encodeURIComponent(keyWord || "_empty")}`;
    const slotStore = await getLocalAsync([slotStorageKey]);
    if (slotStore[slotStorageKey] === slotKey) {
      finish(
        `Supabase：未写入（本 ${minutes} 分钟时间槽已上报，槽键 ${slotKey}，keyWord「${keyWord || "空"}」）`
      );
      return;
    }
  ...
  async function saveRankSnapshot(tabId, payload, done) {
    ...
    const byTab = result[SYCM_RANK_STORAGE_KEYS.rankListByTab] && typeof result[SYCM_RANK_STORAGE_KEYS.rankListByTab] === "object" ? { ...result[SYCM_RANK_STORAGE_KEYS.rankListByTab] } : {};
    const tabKey = String(tabId);
    byTab[tabKey] = payload;
    const meta = byTab[RANK_META_KEY] && typeof byTab[RANK_META_KEY] === "object" ? { ...byTab[RANK_META_KEY] } : {};
    meta[tabKey] = (/* @__PURE__ */ new Date()).toISOString();
  ...
  function initRankBackgroundService() {
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      ...
      if (message.type === SYCM_RANK_RUNTIME.GET_TAB_ID_MESSAGE) {
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
          const tabId2 = tabs && tabs[0] && tabs[0].id != null ? tabs[0].id : null;
          sendResponse({ tabId: tabId2 });
        });
        return true;
      }
      ...
      if (!message.payload || !Array.isArray(message.payload.items) || message.payload.items.length === 0) {
        const reason = message.meta && message.meta.parseError ? String(message.meta.parseError) : "无有效数据";
    ...
    chrome.tabs.onRemoved.addListener(async (tabId) => {
      ...
      const byRank = result[SYCM_RANK_STORAGE_KEYS.rankListByTab] && typeof result[SYCM_RANK_STORAGE_KEYS.rankListByTab] === "object" ? { ...result[SYCM_RANK_STORAGE_KEYS.rankListByTab] } : {};
      const byLogs = result[SYCM_RANK_STORAGE_KEYS.logsByTab] && typeof result[SYCM_RANK_STORAGE_KEYS.logsByTab] === "object" ? { ...result[SYCM_RANK_STORAGE_KEYS.logsByTab] } : {};
      const bySelection = result[SYCM_RANK_STORAGE_KEYS.rankSelectionByTab] && typeof result[SYCM_RANK_STORAGE_KEYS.rankSelectionByTab] === "object" ? { ...result[SYCM_RANK_STORAGE_KEYS.rankSelectionByTab] } : {};
      ...
      if (bySelection[SELECTION_META_KEY] && typeof bySelection[SELECTION_META_KEY] === "object") {
