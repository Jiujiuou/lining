...
function ToolbarPanel({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "rank-toolbar-panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "rank-toolbar-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "rank-toolbar-title", children: "市场排名" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rank-toolbar-actions", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "rank-btn rank-btn-secondary", onClick: onRefresh, children: "刷新列表" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "rank-btn rank-btn-secondary", onClick: onSelectAll, children: "全选" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "rank-btn rank-btn-secondary", onClick: onSelectNone, children: "全不选" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "rank-btn rank-btn-primary", onClick: onSave, children: "保存设置" })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "rank-toolbar-meta", "aria-live": "polite", children: metaText })
  ] });
}
function RankRow({ row, onToggleRow }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rank-list-row", role: "listitem", title: `${row.shopTitle} ${row.itemTitle || ""}`, children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "rank-list-num", children: row.rank }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rank-list-text", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "rank-list-shop", children: row.shopTitle }),
      row.itemTitle ? /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "rank-list-item", children: row.itemTitle }) : null
    ] })
  ] });
}
function RankListPanel({ rows, onToggleRow }) {
  if (!rows || rows.length === 0) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "rank-list-empty", children: "暂无数据。请在生意参谋触发 rank.json 后点击“刷新列表”。" });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "rank-list-panel", role: "list", children: rows.map((row) => /* @__PURE__ */ jsxRuntimeExports.jsx(RankRow, { row, onToggleRow }, row.id)) });
}
function pad2(num) {
  return (num < 10 ? "0" : "") + num;
}
...
function LogItem({ entry }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `rank-log-item rank-log-item--${level}`, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "rank-log-time", children: formatLogTime(entry && entry.t) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "rank-log-text", children: entry && entry.msg ? String(entry.msg) : "" })
  ] });
}
function LogPanel({ logs, onClearLogs }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "rank-log-panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "rank-log-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "rank-log-title", children: "日志" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "rank-log-clear", onClick: onClearLogs, children: "清空" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "rank-log-list", role: "log", "aria-live": "polite", children: !logs || logs.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "rank-log-empty", children: "暂无日志" }) : logs.map((entry, index) => /* @__PURE__ */ jsxRuntimeExports.jsx(LogItem, { entry }, `${entry.t || "time"}-${index}`)) })
  ] });
}
function hasRuntime() {
...
function hasStorageChange() {
  return typeof chrome !== "undefined" && chrome.storage && chrome.storage.onChanged && typeof chrome.storage.onChanged.addListener === "function";
}
...
function removeLocal(keys, callback) {
  const done = typeof callback === "function" ? callback : () => {
  };
  if (!hasStorageLocal()) {
    done();
...
function addStorageChangeListener(listener, areaName = "local") {
...
function useChromeStorageChange(onChange, options = {}) {
  const areaName = options.areaName || "local";
  const enabled = options.enabled !== false;
  const keyList = reactExports.useMemo(() => normalizeKeys(options.keys), [options.keys]);
...
function useTabLogs(options = {}) {
  ...
  const storageKeys = reactExports.useMemo(() => {
...
const SELECTION_META_KEY$1 = "__meta";
...
function getSelectionIds(filter) {
...
function filterIdsToCatalog(ids, items) {
  const catalogSet = /* @__PURE__ */ new Set();
...
function pruneSelectionByTab(byTab) {
  ...
  const next = { ...byTab };
  const meta = next[SELECTION_META_KEY$1] && typeof next[SELECTION_META_KEY$1] === "object" ? { ...next[SELECTION_META_KEY$1] } : {};
  const tabIds = Object.keys(next).filter((key) => key !== SELECTION_META_KEY$1);
  if (tabIds.length <= SYCM_RANK_LIMITS.RANK_MAX_TABS) {
    next[SELECTION_META_KEY$1] = meta;
    return next;
  }
...
function useRankSelectionPersistence(tabId) {
  return reactExports.useCallback(
    (ids) => {
      ...
      getLocalAsync([SYCM_RANK_STORAGE_KEYS.rankSelectionByTab]).then((result) => {
        const byTab = result[SYCM_RANK_STORAGE_KEYS.rankSelectionByTab] && typeof result[SYCM_RANK_STORAGE_KEYS.rankSelectionByTab] === "object" ? { ...result[SYCM_RANK_STORAGE_KEYS.rankSelectionByTab] } : {};
        const tabKey = String(tabId);
        byTab[tabKey] = { itemIds: normalized };
        const meta = byTab[SELECTION_META_KEY] && typeof byTab[SELECTION_META_KEY] === "object" ? { ...byTab[SELECTION_META_KEY] } : {};
        meta[tabKey] = (/* @__PURE__ */ new Date()).toISOString();
...
function buildMetaText(snapshot) {
  const keywordText = snapshot && snapshot.keyWord != null && String(snapshot.keyWord).trim() !== "" ? `搜索词：${String(snapshot.keyWord)}` : "搜索词：（空，未带 keyWord）";
  const updateText = snapshot && snapshot.updateTime ? `接口更新：${snapshot.updateTime}` : "";
  return [keywordText, updateText].filter(Boolean).join(" · ");
}
function useRankSnapshotLoader({
  tabId,
  selectedIds,
  sessionTouched,
  persistSelection,
  ...
  return reactExports.useCallback(() => {
    getLocalAsync([
      ...
      const snapshotByTab = tabId != null && rankByTab[String(tabId)] ? rankByTab[String(tabId)] : null;
      const nextSnapshot = snapshotByTab || result[SYCM_RANK_STORAGE_KEYS.rankListLatest] || { items: [] };
      ...
      const selectionFilter = tabId != null && selectionByTab[String(tabId)] ? selectionByTab[String(tabId)] : result[SYCM_RANK_STORAGE_KEYS.rankSelection];
...
function useRankPopupActions({
  ...
  const onToggleRow = reactExports.useCallback(
    (rowId, checked) => {
  ...
  const onSelectAll = reactExports.useCallback(() => {
    const items = Array.isArray(snapshot.items) ? snapshot.items : [];
    const next = items.map((row, index) => rowKey(row, index)).slice(0, SYCM_RANK_LIMITS.RANK_MAX_ITEMS);
    setSessionTouched(true);
    setSelectedIds(next);
    persistSelection(next);
  }, [persistSelection, setSelectedIds, setSessionTouched, snapshot.items]);
  const onSelectNone = reactExports.useCallback(() => {
    setSessionTouched(true);
    setSelectedIds([]);
    persistSelection([]);
  }, [persistSelection, setSelectedIds, setSessionTouched]);
  ...
  return reactExports.useMemo(() => {
    ...
    return items.map((row, index) => {
      ...
      return {
        ...
        shopTitle: row.shopTitle != null && String(row.shopTitle).trim() !== "" ? String(row.shopTitle).trim() : "（无店名）",
        itemTitle: row.itemTitle != null && String(row.itemTitle).trim() !== "" ? String(row.itemTitle).trim() : "",
        checked: checkedSet.has(id)
      };
    });
  }, [selectedIds, snapshotItems]);
}
function useMarketRankPopupController() {
  const [snapshot, setSnapshot] = reactExports.useState({ items: [], keyWord: "", updateTime: "" });
  ...
  const [sessionTouched, setSessionTouched] = reactExports.useState(false);
  const saveMessageTimerRef = reactExports.useRef(null);
...
function PopupPage() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rank-popup-layout", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "rank-main-panel", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        ToolbarPanel,
        {
          metaText,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(RankListPanel, { rows, onToggleRow })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(LogPanel, { logs, onClearLogs })
  ] });
}
function mountPopup() {
  const container = document.getElementById("root");
  if (!container) {
    console.error("[Rext][extension-sycm-market-rank] 未找到 #root，Popup 挂载失败");
    return;
  }
