(function() {
  "use strict";
  function initFlowSourcePoller() {
    if (globalThis.__LINING_SYCM_DETAIL_FLOW_POLLER__) return;
    globalThis.__LINING_SYCM_DETAIL_FLOW_POLLER__ = true;
    (function() {
      ...
      var templateUrl = "";
      var intervalMs = 30 * 1e3;
      var pad = typeof window !== "undefined" && window.__SYCM_TIME_MAIN__ && typeof window.__SYCM_TIME_MAIN__.pad === "function" ? window.__SYCM_TIME_MAIN__.pad : function(n) {
      ...
      function extractFlowMetrics(data) {
        ...
        function numVal(node, key) {
          var v = node && node[key] && typeof node[key].value !== "undefined" ? Number(node[key].value) : null;
      ...
      function formatRate(n) {
        ...
        var v = Number(n);
        if (v !== v) return String(n);
        return (Math.round(v * 1e4) / 100).toFixed(2) + "%";
      }
      ...
      function fetchOne(itemId) {
        ...
        return fetch(url, { method: "GET", credentials: "include" }).then(function(res) {
          ...
          if (!r.ok || !r.json || r.json.code !== 0) {
            var code = r.json && typeof r.json.code !== "undefined" ? r.json.code : "—";
            var message = r.json && (r.json.message || r.json.msg || r.json.subMsg) ? String(r.json.message || r.json.msg || r.json.subMsg) : "";
            var msg = "item " + String(itemId) + " 请求失败：HTTP " + r.status + " code=" + code + (message ? " message=" + message : "");
          ...
          emitLog(
            "log",
            "[Sycm][轮询][详情] item " + String(itemId) + " │ 搜索UV=" + formatMetric(payload.search_uv) + " 搜索支付转化率=" + formatRate(payload.search_pay_rate) + " │ 购物车UV=" + formatMetric(payload.cart_uv) + " 购物车支付转化率=" + formatRate(payload.cart_pay_rate)
          );
      ...
      function drain() {
        ...
        while (inFlight < maxConcurrency && queue.length > 0) {
          ...
          inFlight++;
          fetchOne(itemId).then(function() {
          }).catch(function(err) {
            stopInternal(
              "请求失败，请打开任意商品详情页刷新模板后重试。错误：" + String(err && err.message || err)
            );
          }).finally(function() {
            inFlight--;
      ...
      function start(opts) {
        ...
        templateUrl = typeof opts.templateUrl === "string" ? opts.templateUrl : templateUrl;
        intervalMs = typeof opts.intervalMs === "number" && opts.intervalMs >= 5e3 ? opts.intervalMs : intervalMs;
        maxConcurrency = typeof opts.maxConcurrency === "number" && opts.maxConcurrency > 0 ? opts.maxConcurrency : maxConcurrency;
        ...
        emitLog(
          "log",
          "[Sycm][轮询] 已开始：商品 " + ids.length + " 个，间隔 " + Math.round(intervalMs / 1e3) + " 秒，并发 " + maxConcurrency
        );
        tick();
      }
      function stop() {
...
//# sourceMappingURL=flow-source-poller.js.map
